...
export type ValoresFiltrosAuditoria = { dtIni: number; dtFim: number; acao: string; usuario: string; tabela: string; saida: string };
...
export function Filtros({ v, erros }: { v: ValoresFiltrosAuditoria; erros: ErrosFiltrosAuditoria }) {
  return (
    <form
      ...
      key={`${v.dtIni}|${v.dtFim}|${v.acao}|${v.usuario}|${v.tabela}|${v.saida}`}
      role="search"
      method="get"
    ...
    >
      <DataLegada name="dtIni" id="filtro-dt-ini" label="Data inicial" defaultValue={v.dtIni || undefined} erro={erros.dtIni} />
      <DataLegada name="dtFim" id="filtro-dt-fim" label="Data final" defaultValue={v.dtFim || undefined} erro={erros.dtFim} />
